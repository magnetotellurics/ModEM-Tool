function [status] = writeZ_3D_simple(cfile,allData,header,units,isign,APRES)
%  Usage:  [status] = writeZ_3D_simple(cfile,allData,header,units,isign,APRES);
...
%   Mod. in Dec 2016 to allow a data object instead of data structure.
%   A "simple" version of writeZ_3D which assumes the same set of
%   sites for all periods.
%  (c) Anna Kelbert, 2017
...
for j = 1:nTx
    disp(['Regrouping frequency number ' num2str(j)]);
    for i = 1:length(allsites)
            ...
            if isprop(allData{j},'TF')
                info.data(i,j,:) = allData{j}.TF(i,:);
                info.err(i,j,:) = allData{j}.TFerr(i,:);
            else
                info.data(i,j,:) = allData{j}.Z(i,:);
                info.err(i,j,:) = allData{j}.Zerr(i,:);
            end
