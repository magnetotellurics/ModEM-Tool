%% Class constructor Latitude-Longitude version
function obj = Grid3DdataTEST2(GRID3D, BGMODEL, DATA1D, DATA2D, DATA3D, varargin)
% INPUTS
...
% ListData from BGMODEL can be created by the function TBGSModel.makeListData
    ...
    xShift = GRID3D.origin.x; % position of the 1st x coord
    yShift = GRID3D.origin.y; % position of the 1st y coord
    R = earthRadius;
    flag1 = 0; flag2 = 0;
    ...
    L3D = length(DATA3D);
    for l = 1:L1D
        obj.DATA1D{l} = NaN(0,0);
    end
    for l = 1:L2D
        obj.DATA2D{l} = NaN(0,0);
    end
    for l = 1:L3D
        obj.DATA3D{l} = NaN(0,0);
    end; % initialize
    ...
    for i = 1:GRID3D.Nx
        ...
        for j = 1:GRID3D.Ny
            ...
            for l = 1:L2D
                temp = DATA2D(l);
                indXY2D{l} = temp.findLatLong([xUTM yUTM]);
            end
            
            % finds possible XY coordinates from BGModel
            temp = GCD([BGMODEL.Lat BGMODEL.Long], [xUTM yUTM],R);
            indXYBG = find(temp == min(temp));
            ...
            for k = 1:GRID3D.Nz
                ...
                for l = 1:L1D
                    temp = DATA1D(l);
                    temp2 = GCD([temp.Lat temp.Long],[xUTM yUTM],R);
                    temp3 = temp.Z - zDepth;
                    ss = sum(temp2.^2) + temp3.^2;
                ...
                for l = 1:L2D
                    temp = DATA2D(l);
                    indZ = temp.findZ(indXY2D{l},zDepth);
                    temp2 = GCD([temp.Lat(indZ) temp.Long(indZ)],...
                        [xUTM yUTM],R);
                    temp3 = temp.Z(indZ) - zDepth;
                    d2(l) = sqrt(sum(temp2.^2) + temp3.^2);
...
%                 L = 0; an another way to combine data
                ...
                temp2 = abs(BGMODEL.Z-zDepth);
                indZBG = find(temp2(indXYBG) == min(temp2(indXYBG)));
                indminBG = indXYBG(indZBG);
                if length(indminBG) > 1 % checks possible errors
                	flag2 = 1;
                end
                indBG = indminBG(1);
