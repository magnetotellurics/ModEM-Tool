classdef mtdatacompare < mtdata
    ...
    methods
        ...
        function [rms,info,freq,site] = misfit(obj,maxrms,fplot,prange)
            ...
            % convert all to column vectors (important for tables)
            info.lon = info.lon(:);
            info.lat = info.lat(:);
            info.per = info.per(:);
            ...
            % Plot
            figure('Position',[300,300,1200,1200],...
                'PaperPosition',[1,1,12,12],...
            ...
            ylabel('RMS');
            legend(info.comp,'Location','BestOutside');
            if nargin > 2
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function [rms,info,freq,site] = apresrms(obj,maxrms,fplot,prange)
            
            % [rms,info,freq,site] = apresrms(obj,maxrms,fplot,prange)
            %
            % Outputs the RMS misfit for two data objects. Plots a summary.
            % Apparent resistivities only.
            ...
            info = obj.v;
            values = obj.apres;
            resxy = abs(values.xy_re - values.xy)./values.xy_se;
            resyx = abs(values.yx_re - values.yx)./values.yx_se;
            compchar = ['RHOXY';'RHOYX'];
            nper = length(info.per);
            nsites = length(info.code);
            ncomp = 2;
            ...
            for j = 1:nper
                ...
                freq.p(j) = info.per(j);
                freq.d(j) = info.per(j)/(24*3600);
                count = nsites - sum(isnan(resxy(:,j)));
                freq.xyrms(j) = sqrt((nansum(resxy(:,j)))/count);
                count = nsites - sum(isnan(resyx(:,j)));
                freq.yxrms(j) = sqrt((nansum(resyx(:,j)))/count);
            end
            ...
            figure('Position',[300,300,1200,1200],...
                ...
                'PaperOrientation','Portrait');
            subplot(7,1,1:5);
            htitle = ['AP. RES. ONLY RMS ' num2str(rms) ' (averaged over ' num2str(nper) ' periods)'];
            InterpPlot(site.lon,site.lat,site.rms,'RMS',htitle);
            colormap(invhot);
            if nargin >= 2
                caxis([1 maxrms]);
            ...
            subplot(7,1,6:7);
            h = zeros(1,2);
            h(1)=semilogx(freq.p,freq.xyrms,'x-','color','b'); hold on;
            set(h(1),'LineWidth',2);
            h(2)=semilogx(freq.p,freq.yxrms,'x-','color','r'); hold on;
            set(h(2),'LineWidth',2);
            xlabel('Log(10) T (secs)');
            ylabel('RMS');
            legend(compchar,'Location','BestOutside');
            if nargin > 2
                print('-djpeg95','-r300',[fplot '_apres_rms.jpg']);
            end
            
        end

        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function [rms,info,freq,site] = phaserms(obj,maxrms,fplot,prange)
            
            % [rms,info,freq,site] = phaserms(obj,maxrms,fplot,prange)
            %
            % Outputs the RMS misfit for two data objects. Plots a summary.
            % Phase only.
            ...
            figure('Position',[300,300,1200,1200],...
                ...
                'PaperOrientation','Portrait');
            subplot(7,1,1:5);
            htitle = ['PHASE ONLY RMS ' num2str(rms) ' (averaged over ' num2str(nper) ' periods)'];
            InterpPlot(site.lon,site.lat,site.rms,'RMS',htitle);
            colormap(invhot);
            if nargin >= 2
                caxis([1 maxrms]);
            ...
            legend(compchar,'Location','BestOutside');
            if nargin > 2
                print('-djpeg95','-r300',[fplot '_phase_rms.jpg']);
            end
        ...
        function [f] = plot(obj,comp,iper,fplot)
            ...
            climsre(1) = (nanmin(rmoutliers(datareal))+nanmin(predreal))/2;
            climsre(2) = (nanmax(rmoutliers(datareal))+nanmax(predreal))/2;
            rangere = climsre(2) - climsre(1);
            if isnan(rangere)
                error('One of the data sets is entirely NaN for this period.');
            end

            climsim(1) = (nanmin(rmoutliers(dataimag))+nanmin(predimag))/2;
            climsim(2) = (nanmax(rmoutliers(dataimag))+nanmax(predimag))/2;
        ...
        function [f] = plotrms(obj,maxrms,comp,iper,fplot)
            ...
            if iper > 0
                ...
                f=figure('Position',[100,100,1600,1200],...
                    'PaperPosition',[1,1,16,12],...
                    'PaperOrientation','Portrait');
                %figure(1), clf
