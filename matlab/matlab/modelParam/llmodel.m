...
%     end
    ...
    methods
       ...
       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function [obj,dlogical] = sidetrim(obj,direction,km)
            %
            %   [obj,dlogical] = sidetrim(obj,direction,distance)
            %
            % crop the llmodel on one side (N,S,E,W) by replacing all cells
            % less than a distance (in km) from that boundary by NaNs
            
            % first convert the distance from km to degrees
            kmPerDeg = latlontools.kmPerDeg(mean(obj.grid.lat));
            distance = km/kmPerDeg;
            ...
            minlon(1:length(vlat)) = min(vlon);
            maxlon(1:length(vlat)) = max(vlon);            
            for i=1:length(vlat)
                for k=1:size(obj.v,3) % find the first well-defined layer
                    jj = find(~isnan(squeeze(obj.v(i,:,k))));
                    if ~isempty(jj) 
                        minlon(i) = vlon(jj(1));
                        maxlon(i) = vlon(jj(end));
                        break; 
                    end
                end
            end
            
            minlat(1:length(vlon)) = min(vlat);
            maxlat(1:length(vlon)) = max(vlat);            
            for j=1:length(vlon)
                for k=1:size(obj.v,3) % find the first well-defined layer
                    ii = find(~isnan(squeeze(obj.v(:,j,k))));
                    if ~isempty(ii)
                        minlat(j) = vlat(ii(1));
                        maxlat(j) = vlat(ii(end));
                        break; 
                    end
                end
            end
            
            dlogical = ones(length(vlat),length(vlon));            
            for i=1:length(vlat)
                if contains(direction,'W')
                    j = vlon < minlon(i)+distance;
                    dlogical(i,j) = 0;
                end
                if contains(direction,'E')
                    j = vlon > maxlon(i)-distance;
                    dlogical(i,j) = 0;
                end
            end
            for j=1:length(vlon)
                if contains(direction,'S')
                    i = vlat < minlat(j)+distance;
                    dlogical(i,j) = 0;
                end
                if contains(direction,'N')
                    i = vlat > maxlat(j)-distance;
                    dlogical(i,j) = 0;
                end
            end
        ...
        function [h,X] = pcolor(obj,type,value,lims,P)
           ...
           elseif strcmp(type,'value')
            ...
            elseif strcmp(type,'conductance')
                % this assumes that the depth is sorted in ascending order
                k1 = find(depth>=value(1)); k1 = k1(1);
                k2 = find(depth<=value(2)); k2 = k2(end);
                Cond = zeros(size(squeeze(obj.v(:,:,1))));
                for k=k1:k2
                    Cond = Cond + squeeze(10.^obj.v(:,:,k))*obj.grid.dz(k)*1000;
                end
                X = log10(Cond);
                disp(['Plotting log10 conductance for depth range ' num2str(depth(k1)) ' - ' num2str(depth(k2)) ' km']);
            else
...
%             end
            ...
            if strcmp(type,'conductance')
                clim([1 4]); title(''); grid off; shading flat; colormap hot
            end
        end
        ...
        function write(obj,fname,format,lims)
            ...
            switch lower(format)
                ...
                case {'netcdf','netcdf3','netcdf4'}
                    ...
                    ncid = netcdfiris.open([fname '.nc'],format);
