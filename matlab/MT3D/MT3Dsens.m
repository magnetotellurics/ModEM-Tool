classdef MT3Dsens < Sensitivity
    ...
    methods
        ...
        function obj = MT3Dsens(d,sigma0)
        ...
        %		d = data vector (cell array of structures)
            obj.dataVecClass = 'MT3DZ';
            obj.modelVecClass = 'MT3DmodelParam';
            obj.J_stored = true;
        ...
        %      update full Jacobian
            ...
            %   run Mod3DMT
            Test3D('COMPUTE_J',m_File,d0_File,J_File)
        ...
        function readSens(obj,cfile)
            ...
            for k = 1:nSigma
                ...
                obj.Sens{k} = struct('v',v,'paramType',s, ...
                    'AirCond',AirCond,'dx',dx,'dy',dy,'dz',dz);
                fread(fid,1,'long');
            end
            fclose(fid);
        end
...
%            end
            ...
            kk = 0;
            for k = 1:dOut.NTX
                [nSite,nComp] = size(dOut.d{k}.Z);
                for j = 1:nSite
                    for  l = 1:nComp
                        kk = kk + 1;
                        Zr = sum(sum(sum(obj.Sens{kk}.v .* m.v)));
                        if dOut.d{k}.Cmplx
                            kk = kk + 1;
                            Zi = sum(sum(sum(obj.Sens{kk}.v .* m.v)));
                            dOut.d{k}.Z(j,l)  = Zr+1i*Zi;
                        else
                            dOut.d{k}.Z(j,l)  = Zr;
                        end
                    end
        ...
        %     cell array of data/model space structures)
            ...
            for k = 1:nTx
                [nSite,nComp] = size(d.d{k}.Z);
                for j = 1:nSite
                    for  l = 1:nComp
                        kk = kk + 1;
                        mTemp.v = mTemp.v + real(d.d{k}.Z(j,l))*obj.Sens{kk}.v;
                        if obj.d.d{k}.Cmplx
                            kk = kk + 1;
                            mTemp.v = mTemp.v + imag(d.d{k}.Z(j,l))*obj.Sens{kk}.v;
                        end
                    end
                end
            end
            mOut = MT3DmodelParam(obj.grid,obj.paramType);
            mOut = setModelParam(mOut,mTemp.v,mTemp.paramType,mTemp.AirCond);
        end
        %******************************************************************
        function m =  rowJ(obj,k)
        %   extract row k from representer
            if isa(obj.Sens{k},'MT3DmodelParam')
                m = obj.Sens{k};
            else
                m = MT3DmodelParam();
                m.v = obj.Sens{k}.v;
                m.paramType = obj.Sens{k}.paramType;
                m.AirCond = obj.Sens{k}.AirCond;
                Grid.dx = obj.Sens{k}.dx;
                Grid.dy = obj.Sens{k}.dy;
                Grid.dz = obj.Sens{k}.dz;
                Grid.Nx = length(obj.Sens{k}.dx);
                Grid.Ny = length(obj.Sens{k}.dy);
                Grid.NzEarth = length(obj.Sens{k}.dz);
                Grid.NzAir = [];
                Grid.origin = [0,0,0];
                Grid.rotation = 0;
                Grid.units = 'm';
        ...
        %   complex cases, respectively
            ...
            kk = 0;
            modTemplate = MT3DmodelParam(obj.grid,obj.paramType);
            for k = 1:d.NTX
                ...
                for j = 1:nSite
                    for  l = 1:nComp
                        kk = kk + 1;
                        mReal = mReal + real(d.d{k}.Z(j,l))*obj.Sens{kk}.v;
                        if SaveCmplx
                            mImag = mImag + imag(d.d{k}.Z(j,l))*obj.Sens{kk}.v;
                        end
                        
                        if obj.d.d{k}.Cmplx
                            kk = kk + 1;
                            mReal = mReal + imag(d.d{k}.Z(j,l))*obj.Sens{kk}.v;
                            if SaveCmplx
                                mImag = mImag - real(d.d{k}.Z(j,l))*obj.Sens{kk}.v;
                            end
                        end
                    end
                end
