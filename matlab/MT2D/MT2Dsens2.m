classdef MT2Dsens < Sensitivity
    ...
    methods
        ...
        %    viz:
        %   Write m, d to files to be read by Fortran program Mod2DMT
        ...
        %		d = data vector (cell array of structures)
            ...
            obj.dataVecClass = 'MT2DZ';   %    data vector class for this
                                 %    implementation of sensitivity
            obj.modelVecClass = 'MT2DmodelParam'   %  model parameter class
        end
        %******************************************************************
        ...
        %******************************************************************
        function obj = readSens(obj,cfile)
            %
            ...
            fread(fid,1,'long');
            obj.Sens = cell(nSigma,1);
            
            for k = 1:nSigma
                obj.Sens{k} = MT2DmodelParam(obj.grid,obj.paramType);
                fread(fid,1,'long');
                s = fread(fid,80,'char');
                s = deblank(char(s'));
                fread(fid,2,'long');
                ...
                fread(fid,1,'long');           
                obj.Sens{k} = setModelParam(obj.Sens{k},v,s,AirCond);
            end
            fclose(fid);
        end    
        %******************************************************************
