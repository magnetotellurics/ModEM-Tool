function [xiselect,yiselect,xselect,yselect] = selectcells(varargin)
...
if ~iscell(xdata)
  xdata = xdata(:);
  ydata = ydata(:);
  % compute cell centers and store as points, instead
  xdata = xdata(1:end-1) + diff(xdata)/2;
  ydata = ydata(1:end-1) + diff(ydata)/2;
  ...
  for i = 1:length(xdata)
    ...
    ydata{i} = ydata{i}(:);
    % compute cell centers and store as points, instead
    xdata{i} = xdata{i}(1:end-1) + diff(xdata{i})/2;
    ydata{i} = ydata{i}(1:end-1) + diff(ydata{i})/2;
    [xdata{i},ydata{i}]=meshgrid(xdata{i},ydata{i});
  end
  % total number of data points
...
if strcmpi(params.Return,'unselected')
  if ~iscell(xdata)
    ...
    [xiselect,yiselect]=ind2sub(size(xdata),pointslist);
    ...
    for i = 1:numel(xdata);
      ...
      [xiselect{i},yiselect{i}]=ind2sub(size(xdata),pointslist{i});
