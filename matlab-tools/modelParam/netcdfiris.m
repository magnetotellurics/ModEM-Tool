classdef netcdfiris < handle
    ...
    methods
        function [obj] = netcdfiris(cfile,version)
            %   class constructor
            if nargin < 2
                version = 'netcdf4';
            end
            obj.cfile = cfile;
            ncid = netcdfiris.open(obj.cfile,version);
    ...
    methods(Static)
        ...
        function [ncid] = open(cfile,version)
            
            % use netcdf4 by default
            FORMAT = 'NC_FORMAT_NETCDF4_CLASSIC';
            
            if nargin > 1
                if contains(version,'3')
                    FORMAT = 'NC_FORMAT_CLASSIC';
                end
            end

            netcdf.setDefaultFormat(FORMAT);
        ...
        function [header,geospatial] = getHeader(ncid)
            ...
            try
                header.author_institution = netcdf.getAtt(ncid,ncglobal,'author_institution');
            catch
                try
                    header.author_institution = netcdf.getAtt(ncid,ncglobal,'institution');
                catch exception
                    if strcmp(exception.identifier,'MATLAB:imagesci:netcdf:libraryFailure')
                        header.author_institution = '';
                    end
                end
            end
            try
            ...
            try
                header.reference = netcdf.getAtt(ncid,ncglobal,'references');
            catch
                try
                    header.reference = netcdf.getAtt(ncid,ncglobal,'reference');
                catch exception
                    if strcmp(exception.identifier,'MATLAB:imagesci:netcdf:libraryFailure')
                        header.reference = '';
                    end
            ...
            try
                header.reference_pid = netcdf.getAtt(ncid,ncglobal,'reference_pid');
            catch
                header.reference_pid = '';
            end
            try
        ...
        function [vars,myind] = getValue(ncid,nout)
            ...
            if nvars >= 1
                ...
                for j=1:nvars+ndims
                    ...
                    % skip if this is the missing value, otherwise read on
                    if strcmp(readme,'missing_value')
                        continue
                    end
                    ...
                    try
                        %missing_value = netcdf.getVar(ncid,netcdf.inqVarID(ncid,'missing_value'));
                        %vars(i).missing_value = single(missing_value);
        ...
        function putValue(ncid,vars,myind)
            ...
            for i = 1:length(vars)
                ...
                %fillvaluedim = netcdf.defDim(ncid, 'missing_value', 1);
                %fillvalueid = netcdf.defVar(ncid,'missing_value','float',fillvaluedim);
                ...
                netcdf.endDef(ncid);
                %netcdf.putVar(ncid,fillvalueid,single(missing_value));
           end            
        ...
        function putHeader(ncid,header,geospatial,limits)
            ...
            netcdf.putAtt(ncid,ncglobal,'author_institution',header.author_institution);
            ...
            netcdf.putAtt(ncid,ncglobal,'reference',header.reference);
            netcdf.putAtt(ncid,ncglobal,'reference_pid',header.reference_pid);
  ...
  'repository_institution','',...
    ...
    'vertical_positive', 'down');
             ...
             if nargin > 1
                 if isscalar(res)
                     geospatialInfo.lon_resolution = sprintf('%4.2g',res);
                     geospatialInfo.lat_resolution = sprintf('%4.2g',res);
                 else
                     geospatialInfo.lon_resolution = sprintf('%4.2g',res(1));
                     geospatialInfo.lat_resolution = sprintf('%4.2g',res(2));
                 end
             end
